...
if [[ "$(docker images -q res/reverse-proxy-load-balancer 2> /dev/null)" == "" ]]
then
    echo "Image not found. Building it!"
    docker build --tag res/reverse-proxy-load-balancer $path
fi
...
echo "Running image \"res/reverse-proxy-load-balancer\"."
docker run --rm -p 8080:80 --name res-reverse-proxy-load-balancer -d res/reverse-proxy-load-balancer
